...
class Rotary:
    ...
    def __init__(self,dt,clk,sw):
        self.dt_pin = Pin(dt, digitalio.IN, digitalio.PULL_DOWN)
        self.clk_pin = Pin(clk, digitalio.IN, digitalio.PULL_DOWN)
        self.sw_pin = Pin(sw, digitalio.IN, digitalio.PULL_DOWN)
        self.last_status = (self.dt_pin.value() << 1) | self.clk_pin.value()
        self.dt_pin.irq(handler=self.rotary_change, trigger=digitalio.IRQ_FALLING | digitalio.IRQ_RISING )
        self.clk_pin.irq(handler=self.rotary_change, trigger=digitalio.IRQ_FALLING | digitalio.IRQ_RISING )
        self.sw_pin.irq(handler=self.switch_detect, trigger=digitalio.IRQ_FALLING | digitalio.IRQ_RISING )
        self.handlers = []
