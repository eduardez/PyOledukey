...
class EncoderHandler(Module):
    ...
    def during_bootup(self, keyboard):
        if self.encoder:
            ...
            self.encoder.on_move_do = lambda x: self.on_move_do(
                keyboard, x
            )
            self.encoder.on_button_do = (
                lambda x: self.on_button_do(
                    keyboard, x
                )
            )
        return
    
    def changeLayer(self, new_layer):
        self.map = new_layer
    
    def on_move_do(self, keyboard, state):
        print(self.map)
        if self.map:
            ...
            if state['direction'] == -1:
                ...
                key_index = 1
            key = self.map[key_index]
            keyboard.tap_key(key)

    def on_button_do(self, keyboard, state):
        if state['is_pressed'] is True and self.map:
            print('pressed')
            key = self.map[2]
            keyboard.tap_key(key)
